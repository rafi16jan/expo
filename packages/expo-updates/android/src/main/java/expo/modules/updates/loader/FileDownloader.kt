...
import expo.modules.updates.UpdatesUtils.parseContentDisposition
...
class FileDownloader(context: Context, private val configuration: UpdatesConfiguration) {
  ...
  private fun parseMultipartRemoteUpdateResponse(responseBody: ResponseBody, responseHeaderData: ResponseHeaderData, callback: RemoteUpdateDownloadCallback) {
    ...
    try {
      responseBody
        .takeIf { it.contentLength() > 0 }
        ?.let { MultipartReader(responseBody) }
        ?.use { reader ->
          while (true) {
            ...
            nextPart.use { part ->
              ...
              if (contentDispositionValue != null) {
                val contentDispositionParameterMap = contentDispositionValue.parseContentDisposition()
